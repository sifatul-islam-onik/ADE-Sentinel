...
> **Before you start: the tree has been trimmed.** The project is finished, and the repo
> has been reduced to what the demo needs to run plus every artefact the experiments
> produced. `scripts/`, `tests/`, and five `src/` modules are **no longer in the working
> tree** — they are in git history at commit `b520b51`. Restore them with the command in
> [README.md](README.md#repository-scope--read-this-first).
>
> This document is a **history**, so it still names those files. Where a file below is
> marked ⛏ it lives only in git history; everything unmarked you can open right now.
> The results it describes — `results/`, `report/`, `models/`, `data/splits/`,
> `notebooks/` — are all committed and intact.
...
This is why the repo had `scripts/` (real code) **and** `notebooks/` (glue that runs that code on Kaggle). The notebooks are mostly **auto-generated** — that's what the ⛏`scripts/_gen_*_nb.py` files did. You never edit the `.ipynb` by hand; you edit the generator.

The four notebooks were kept in the trim; their generators were not. If a notebook needs to change, restore the generators first rather than hand-editing build output.
...
31. ⛏`scripts/document_stage1.py`, ⛏`scripts/document_stage2.py`, ⛏`scripts/document_analysis.py` → the `report/*.md` files. **These are generated from the results, not typed** — so the report can never drift from the numbers. The generated `report/*.md` are committed and intact; only the generators were trimmed.
...
|---|---|
| `app/` | The Streamlit demo. The one thing that still runs. |
| `src/` | The 11 modules the demo imports: tokenizer, BIO conversion, both model classes, Stage 2 inference and scoring, and `demo_pipeline.py` that wires the two stages together. |
| `data/splits/` | The frozen train/dev/test files. Committed on purpose. Never regenerate. |
| `models/` | The shared vocabulary, the E0–E3 matrices, and every trained checkpoint the demo loads. |
| `results/` | **Every number the project produced.** `runs.csv` is the master log. |
| `report/` | Markdown write-ups, generated from `results/`. |
| `notebooks/` | The Kaggle glue that ran the remote jobs. Build output — don't hand-edit. |

**Removed in the trim (git history, `b520b51`):**

| Folder | What lived there |
|---|---|
| ⛏`scripts/` | One file per build step. Each one ran, wrote an artefact, exited. Also the four `_gen_*_nb.py` notebook generators. |
| ⛏`tests/` | pytest. Mostly guarded the fiddly bits (tokenizer, BIO conversion). |
| ⛏`src/` (5 files) | `utils.py` (the `log_run` spine), `metrics.py`, `embedding_eval.py`, `challenge_set.py`, `error_sample.py` — used by the scripts and tests, never by the demo. |
...
> Steps 3–7 lean heavily on files marked ⛏, which are not in the working tree. If you
> want to actually read the pipeline rather than just the results, restore them first
> with the command in [README.md](README.md#repository-scope--read-this-first) — it is
> one line and does not touch anything that is currently here. If you only need to
> understand and run the demo, steps 1, 2, 6 and 8 work as written against the tree
> as it stands.
...
8. **⛏`src/utils.py`** — start at `log_run()`. Understand what gets recorded and why (`git_commit`, `device_count`, `effective_batch`).
...
11. **⛏`scripts/build_splits.py`** — how train/dev/test were frozen, and the leakage assertions.
12. **⛏`scripts/fetch_pubmed.py`** — skim only. Just note the year-window trick.
13. **⛏`scripts/prepare_corpus.py`** — skim.

### Step 5 — The headline experiment

14. **⛏`scripts/train_embeddings.py`** — how E2 and E3 were made.
15. **⛏`scripts/build_embedding_matrices.py`** — **read this one properly.** The "only the embedding matrix changes" guarantee lives here.
16. **[src/models/bilstm.py](src/models/bilstm.py)** — the model itself.
17. **⛏`scripts/train_bilstm.py`** — runs 3–6. This is the experiment.
18. **⛏`src/embedding_eval.py`** — the coverage and nearest-neighbour evidence.

### Step 6 — Stage 2

19. **[src/bio_convert.py](src/bio_convert.py)** — read `to_bio()` and `bio_to_entities()`. Read ⛏`tests/test_bio_convert.py` alongside it; the tests explain the tricky cases better than the code does.
...
22. **⛏`scripts/pipeline_eval.py`** — the error-propagation analysis.
23. **⛏`src/challenge_set.py`** + **⛏`scripts/negation_eval.py`** — the negation ladder.
...
27. ⛏`pytest tests/` — used to confirm the environment was right. The suite was removed in the trim; if you restored it in the note above, it runs again once `requirements-dev.txt` is installed.
...
- **Half the files this document names are not in the tree.** That is deliberate, not rot — see the note at the top. Anything marked ⛏ is one `git checkout b520b51 -- ...` away.
- **The `notebooks/*.ipynb` files are build output.** Don't edit them. Edit ⛏`scripts/_gen_*_nb.py` and regenerate — which means restoring the generators first.
- **The `report/*.md` files are also generated**, by ⛏`scripts/document_*.py`. Don't hand-edit those either. The generated files are still committed; regenerating them needs the scripts back.
- **`data/splits/` being in git is deliberate**, not a mistake. `README.md` calls this out explicitly.
- **`requirements-local.txt` has no PyTorch on purpose.** That made sense when every neural run happened on Kaggle. Now that the demo is the thing that runs locally, `requirements-dev.txt` is **required**, not an extra — it carries the CPU torch build, `transformers`, `pytorch-crf` and `streamlit`.
