...
cells = [
    ...
    md("## 1. Environment, and the single-GPU pin (PLAN F8)",
       ...
       "### The two installs",
       "",
       "`pytorch-crf` and `seqeval` are GROUP A of `requirements-remote.txt` - pinned,",
       "and not on the Kaggle base image.",
       "",
       "They go in through `sys.executable -m pip`, not `!pip`. On Kaggle the bare",
    ...
    code("import os",
         ...
         "     'pytorch-crf==0.7.2', 'seqeval==1.2.2'],",
         "    capture_output=True, text=True)",
         "if result.returncode:",
         "    print(result.stdout[-4000:])",
         "    print(result.stderr[-4000:])",
         "    raise SystemExit('pip install failed - see the output above')",
         "print('installed : pytorch-crf, seqeval')",
         "",
         "import torch, transformers, datasets, numpy",
         "import seqeval, torchcrf   # the two just installed - fail here, not mid-run",
         "from importlib.metadata import version",
         "",
         "print('torch', torch.__version__, '| transformers', transformers.__version__)",
         "# seqeval exposes no __version__ attribute; ask the package metadata.",
         "print('seqeval', version('seqeval'), '| pytorch-crf', version('pytorch-crf'))",
         "",
